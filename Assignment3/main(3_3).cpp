...
void allocateColor(vec3 a, vec3 b, vec3 c, vec3 d, unsigned int coli);
void setVertices();
void display();
void idle();
...
int main()
{
	...
	window = glfwCreateWindow(512, 512, "assign03-3", NULL, NULL);	// window 생성
...
void quad(vec3 a, vec3 b, vec3 c, vec3 d)
{
	...
	vec3 dd = vec3(d*scale);
	if (scale > 0.6f) {
		allocateColor(aa, bb, cc, dd, cnt);
		cnt++;
		quad(a, b, c, d);
	}
	else {
...
void setVertices()
{
	...
	quad(baseVertices[0], baseVertices[1], baseVertices[2], baseVertices[3]);

	cnt = 0;
	quad(baseVertices[4], baseVertices[5], baseVertices[6], baseVertices[7]);
